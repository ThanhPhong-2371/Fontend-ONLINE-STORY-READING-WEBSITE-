...
const AdminUserManagement = () => {
    ...
    const getRoleBadgeStyle = (roleName) => {
        switch (roleName) {
            case 'ADMIN': return 'bg-rose-50 text-rose-600 border-rose-100 ring-4 ring-rose-500/5';
            case 'STAFF': return 'bg-indigo-50 text-indigo-600 border-indigo-100 ring-4 ring-indigo-500/5';
        ...
        <AdminLayout>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                <div className="space-y-1">
                    ...
                    <p className="text-slate-500 font-medium font-sans mt-2">Kiểm soát danh tính, phân quyền và trạng thái hoạt động của thành viên.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {loading ? (
                    ...
                    users.map(user => (
                        <div key={user.id} className={`group bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-indigo-500/5 transition-all duration-500 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden ${!user.active ? 'opacity-75 grayscale-[0.5]' : ''}`}>
                            ...
                            <div className="flex-1 min-w-0 text-center md:text-left">
                                <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                                    ...
                                    <div className="flex gap-2">
                                        {user.roles && user.roles.map(role => (
                                            <Badge key={role.id} variant="outline" className={`text-[10px] py-0.5 px-3 rounded-full font-black uppercase tracking-wider border ${getRoleBadgeStyle(role.name)}`}>
                                                {role.name}
                                            </Badge>
                                        ))}
                                    </div>
                                ...
                                <div className="flex items-center justify-center md:justify-start gap-2">
                                    <code className="text-[10px] bg-indigo-50 text-indigo-500 px-3 py-1 rounded-full font-bold border border-indigo-100">ID: #{user.id}</code>
                                    <code className={`text-[10px] px-3 py-1 rounded-full font-bold border transition-colors ${user.active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                                        {user.active ? 'MÁY CHỦ_ACTIVE' : 'MÁY CHỦ_LOCKED'}
                                    </code>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-row md:flex-col gap-2 shrink-0 relative z-10">
                                ...
                                <Button
                                    ...
                                    className={`h-11 w-11 rounded-2xl transition-all ${user.active ? 'text-slate-400 hover:bg-rose-50 hover:text-rose-600' : 'text-rose-500 bg-rose-50 hover:bg-emerald-50 hover:text-emerald-600'}`}
                                    title={user.active ? "Khóa tài khoản" : "Mở khóa"}
                                >
                                    {user.active ? <Unlock className="h-5 w-5" /> : <Lock className="h-5 w-5" />}
                                </Button>
                                <Button
                                    variant="ghost"
                                    size="icon"
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in transition-all">
                    <div className="w-full max-w-xl bg-white rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] p-8 md:p-10 animate-in zoom-in-95 duration-500 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => setIsInfoModalOpen(false)} className="rounded-2xl h-12 w-12 bg-slate-50 text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors">
                                <X size={24} />
                            </Button>
                        </div>
                        ...
                        <form onSubmit={handleInfoSubmit} className="space-y-6">
                            ...
                            <div className="flex gap-4 justify-end pt-6 border-t border-slate-100 mt-6">
                                <Button type="button" variant="ghost" className="h-12 px-8 rounded-2xl font-bold text-slate-400" onClick={() => setIsInfoModalOpen(false)}>Hủy bỏ</Button>
                                <Button type="submit" className="h-12 px-12 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black shadow-xl shadow-indigo-100 transition-all hover:-translate-y-1">Lưu hồ sơ 🚀</Button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in transition-all">
                    <div className="w-full max-w-xl bg-white rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] p-8 md:p-10 animate-in zoom-in-95 duration-500 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => setIsRoleModalOpen(false)} className="rounded-2xl h-12 w-12 bg-slate-50 text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors">
                                <X size={24} />
                            </Button>
                        </div>
                        ...
                        <form onSubmit={handleRoleSubmit}>
                            <div className="space-y-4 mb-8">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {roles.map(role => (
                                        ...
                                        >
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${formData.roleIds.includes(role.id)
                                            ...
                                            <div className="flex-1">
                                                <span className={`text-base font-black tracking-tight block uppercase ${formData.roleIds.includes(role.id) ? 'text-indigo-800' : 'text-slate-600'}`}>
                                                    {role.name}
                                                </span>
                                            ...
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${formData.roleIds.includes(role.id)
                            ...
                            <div className="flex gap-4 justify-end pt-6 border-t border-slate-100">
                                <Button type="button" variant="ghost" className="h-12 px-8 rounded-2xl font-bold text-slate-400" onClick={() => setIsRoleModalOpen(false)}>Hủy bỏ</Button>
                                <Button type="submit" className="h-12 px-12 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black shadow-xl shadow-indigo-100 transition-all hover:-translate-y-1">Xác nhận cấp quyền 🚀</Button>
                            </div>
                        </form>
                    </div>
                </div>
